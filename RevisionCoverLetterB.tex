...
Below, we address each reviewer's comments point-by-point. All changes have been highlighted with red text in the revised manuscript, and all line numbers and equation numbers also refer to the revised manuscript.
...
This has been fixed. The matrix we are discussing on lines 88-89 is $-i\lambda_0 P_n'$.
...
The text ``see subsection 2.2 for more details'' is added after the formula on line 94.
...
We added a note about how $n\ge2$ can be reformulated as an $n=1$ problem (lines 109-110).
...
A sentence has been added in line 114 which introduces the first case ($A_0$ nonsingular). A sentence has been added in line 123 which introduces the second case ($A_0$ singular).
...
This is now explained on line 366.
...
Theorem 3.1 has been rewritten.  We hope that the new version better reflects what the referees would like to see.

\item \emph{Page 12, lines 401-403. The relation between $c$ and $c_0$ is missing.}
\vspace{4mm}

In line 425, we now explain that $c$ is a free parameter typically representing the wavespeed. In line 439, we define the zero-amplitude wavespeed $c_0$.
...
This was a typo and has been fixed (lines 484 and 486).
...
This is now explained starting on line 496. An appropriate reference is cited.
...
A brief explanation has been added, starting on line 515.
...
We agree with the referee and regret that we got the ordering wrong. We moved this discussion up to Remark 4.3.

\item \emph{Page 18, lines 609-610. It is not specified what is $A_0$ in the presented example.}
\vspace{4mm}

$A_0$ is now defined on line 656.
...
Agree. The text now states (starting on line 666) that the index is unchanged if one stays away from zero and $\mu_{ch}$. See also Remark 4.5.
...
It was indeed a typo.  The corrected statement is on lines 709-711 of the revision.
...
Since this is a generic setting with Hilbert space, $X$, we are using the norm induced from the inner product.  This is mentioned at the beginning of the article.
...
The Hilbert space is now specified for the linear operator $A_0(U^*)$ (equation (6.20)). The Hilbert space is also specified for the operator $\mathcal{P}_2(\lambda; U_n)$ (equation (6.26)). All norms and inner products that will be used in the section are specified before the statement of Theorem 6.6 (formerly Theorem 6.4).
...
We added text in line 938-939 saying that it is our own numerical evidence which supports Hypothesis 6.5. (Hypothesis 6.5 has been restated in a more general form in this revision). See also Remark 6.7, where we also mention our own numerical evidence. 
...
Theorem 6.6 (formerly Theorem 6.4) states that the bounds for $r$ and $w_j$ hold for all derivatives with respect to $x$. As recommended by the reviewer, we have added the particular case of $||\partial_x w||_\infty$, since that result is used later (lines 974-975).
...
This proof of lemma 6.19 has been completely rewritten, and this point should now be clear. In particular, we are able to remove the projection term since $\tilde{w} \in S^\perp$.
...
$H(x)$ is a remainder term. We now explain precisely where this term comes from after the system of equations (6.64).
...
This now also follows from Lemma 6.4 (added in this revision), since both $\partial_c U$ and $\partial_x U$ are exponentially localized.

\item \emph{Page 39, line 1234. I do not see how the factor $2c$ (line 1233) changed to $c$ (line 1234).}
\vspace{4mm}

This is a typo and has been fixed (line 1446).
...
Instability bubbles are now briefly described at the beginning of section 4 (lines 415-418).

\item \emph{What is sideband instability?}
\vspace{4mm}

A sideband instability is unstable spectra associated with nonzero $\mu$. This is now explained on line 492-493, and we take $\mu > 0$.

\item \emph{Why is knowing small eigenvalues important?}
\vspace{4mm}

We have added several sentences to this effect at the beginning of Section 5 (starting on line 719).
