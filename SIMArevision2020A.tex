...
\]
Since $e^{(\alpha - \epsilon)r(x)} F_c(Y(x;c); c) \in \Ran \calL_{\alpha - \epsilon}$, the set of all bounded solutions to \cref{Zcprime} is $Z_c(x; c) + \R e^{(\alpha - \epsilon)r(x)} F_c(Y(x;c); c)$. This implies that $Z_c(x; c)$ is bounded. It follows from \cref{defYZ} that 
\begin{align*}
...
\begin{remark}$\calA_0(U_n)$ may in fact have additional eigenvalues $\lambda$ with $\lambda > \delta_0 > 0$, but these do not matter for the analysis. Our numerical evidence suggests that there are no additional eigenvalues.
\end{remark}
